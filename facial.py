...
with st.sidebar:
    ...
    mirror_feed = st.checkbox("Mirror Feed", value=True)
    if mode == "Video Mode":
...
if mode == "Video Mode":
    ...
    if image is not None:
        ...
        processed = process_single_image(img_bgr, mirror=mirror_feed)
...
with st.sidebar:
    ...
    if mode == "Video Mode":
        ...
        mirror_snap = st.checkbox("Mirror Image", value=True)
...
def predict_and_annotate_single(img_bgr):
    face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')
    ...
    for (x, y, w, h) in faces:
        # Emotion detection (PyTorch)
        face_emotion = gray[y:y+h, x:x+w] if in_channels == 1 else img_bgr[y:y+h, x:x+w]
        face_emotion = cv2.resize(face_emotion, (48, 48))
        if in_channels == 3:
            face_emotion_rgb = cv2.cvtColor(face_emotion, cv2.COLOR_BGR2RGB)
            face_emotion_pil = Image.fromarray(face_emotion_rgb, mode='RGB')
        ...
        gender = "unknown"
        if age_gender_model is not None:
            face_age_gender = img_bgr[y:y+h, x:x+w]
            face_age_gender = cv2.resize(face_age_gender, (64, 64))
            face_age_gender = face_age_gender / 255.0
            face_age_gender = np.expand_dims(face_age_gender, axis=0)
            try:
        ...
        # Draw rectangle for face
        cv2.rectangle(img_bgr, (x, y), (x+w, y+h), (255, 255, 255), 2)

        # Display emotion
        emotion_color = emotion_colors.get(emotion, (255, 0, 0))
        text_size_emotion = cv2.getTextSize(emotion, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)[0]
        cv2.rectangle(img_bgr, (x, y-75), (x+text_size_emotion[0], y-45), (255, 255, 255), -1)
        cv2.putText(img_bgr, emotion, (x, y-50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, emotion_color, 2)

        # Display age (continuous, smoothed)
        age_text = f"Age: {age}"
        text_size_age = cv2.getTextSize(age_text, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)[0]
        cv2.rectangle(img_bgr, (x, y-45), (x+text_size_age[0], y-15), (255, 255, 255), -1)
        cv2.putText(img_bgr, age_text, (x, y-20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, age_color, 2)

        # Display gender
        gender_color = gender_colors.get(gender, (255, 0, 0))
        text_size_gender = cv2.getTextSize(gender, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)[0]
        cv2.rectangle(img_bgr, (x, y-15), (x+text_size_gender[0], y+15), (255, 255, 255), -1)
        cv2.putText(img_bgr, gender, (x, y+10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, gender_color, 2)
...
if mode == "Video Mode":
    ...
    if uploaded_file is not None:
        ...
        annotated_img = predict_and_annotate_single(img_bgr)
